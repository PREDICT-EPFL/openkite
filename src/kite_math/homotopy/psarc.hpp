...
symbolic_psarc<Equalities, CorrectorProps>::symbolic_psarc(const typename Equalities::num &init_guess, const CorrectorProps &props)
{
    ...
    typename Equalities::sym tang  = Equalities::sym::sym("tang", x.size1(), 1);       // tangent vector to a solution curve
    typename Equalities::sym ds = Equalities::sym::sym("ds");                   // step length along the arc
    typename Equalities::sym u0 = Equalities::sym::sym("x0", x.size1(), 1);
    typename Equalities::sym plane_constraint = Equalities::sym::dot(tang, x - u0) - ds;
    ...
    casadi::DMDict ARG;
    NLP["x"] = x;
    NLP["f"] = 0; //0.5 * Equalities::sym::dot((x - w), (x - w));
    NLP["g"] = Equalities::sym::vertcat({homotopy, plane_constraint});
    NLP["p"] = Equalities::sym::vertcat({tang,u0,ds});
    ...
    std::cout << "Reached corrector constructor \n";
    ...
    typename Equalities::num lbg = Equalities::num::zeros(homotopy.size1() + 1);
    ...
    ARG["lbg"] = lbg;
    ARG["ubg"] = ubg;
    ARG["p"] = Equalities::num::vertcat({Equalities::num::zeros(x.size1(), 1),0}); // t = 0, sd = 0; - just solve homotopy
    ...
    typename Equalities::num x0_num = ARG["x0"]; //solution.at("x");
    ...
    solution["x"]     = ARG["x0"];
    solution["lam_g"] = Equalities::num::zeros(ARG["lbg"].size1());
    solution["lam_x"] = Equalities::num::zeros(ARG["x0"].size1());
    ...
    while(lambda_val > 0.0)
    {
        ...
        ARG["p"]      = Equalities::num::vertcat({t,x0_num,h});
        ARG["x0"]     = x_next;
