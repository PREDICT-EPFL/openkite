...
{	
    ...
    casadi::DM id_data    = casadi::DM::zeros(state_size, DATA_POINTS);
    ...
    //                                             2.5189837e+00,  -7.3947045e-01,  -2.1751468e-01,   3.6023340e-01,   5.4059382e-01,  -1.8748153e-01,  -7.3681384e-01});
    //casadi::DM init_guess = casadi::DM::repmat(init_state, 101, 1);
    casadi::DM tmp = casadi::DM::zeros(id_data.size1(), id_data.size2());
    int counter = 0;
    for(int i = id_data.size2() - 1; i >= 0; i--)
    {
        tmp(casadi::Slice(0, id_data.size1()), counter) = id_data(casadi::Slice(0, id_data.size1()), i);
        counter++;
    }

    casadi::DM init_guess = casadi::DM::vec(tmp);
