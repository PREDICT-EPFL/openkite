...
template<int PolyOrder, int NumSegments, int NX, int NU>
PSODESolver<PolyOrder, NumSegments, NX, NU>::PSODESolver(casadi::Function ODE, const float &dt,
                                                         const casadi::DMDict &props) {
    scale = 0;
    P = casadi::DM::eye(NX);
    R = casadi::DM::eye(NU);
    ...
    ARG["x0"] = casadi::DM::vertcat(
            casadi::DMVector{casadi::DM::repmat(feasible_state, (NumSegments * PolyOrder + 1), 1),
                             casadi::DM::repmat(feasible_control, (NumSegments * PolyOrder + 1), 1)});
}

template<int PolyOrder, int NumSegments, int NX, int NU>
casadi::DM
PSODESolver<PolyOrder, NumSegments, NX, NU>::solve(const casadi::DM &X0, const casadi::DM &U, const bool full) {
...
casadi::DMDict PSODESolver<PolyOrder, NumSegments, NX, NU>::solve_trajectory(const casadi::DM &X0,
    ...
    if (U.size1() != (NumSegments * PolyOrder + 1) * NU) {
        std::cout << "Control vector size should be " << (NumSegments * PolyOrder + 1) * NU << " provided: "
                  << U.size1() << "\n";
        casadi::DMDict fault;
        fault["x"] = X0;
